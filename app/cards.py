...
def render_executive_summary(risks, evidence_pool, requires_hitl):
    ...
    with date_col:
        st.markdown(
            f"<div style='text-align: right; font-family: JetBrains Mono; font-size: 13px; color: #45464d;'>Analysis Timestamp: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}</div>",
    ...
    docs_analyzed = (
        len(set(c.get("location", "unknown") for c in evidence_pool))
        if evidence_pool
        else 0
    )
...
def render_risk_breakdown(risks, evidence_pool):
    ...
    if not risks:
        st.info(
            "No delivery risks matched the criteria in the provided source documents."
        )
        return
    ...
    for idx, r in enumerate(risks, 1):
        with st.container(border=True):
            ...
            st.markdown(
                f"<br>**EXPLANATION**<br>{r.get('explanation', 'No explanation provided.')}",
                unsafe_allow_html=True,
            )
            ...
            if impact:
                ...
                for col, (title, val) in zip([imp_c1, imp_c2, imp_c3, imp_c4], metrics):
                    with col:
                        st.markdown(
                            f"<div style='background-color: #F8FAFC; padding: 1rem; border: 1px solid #E2E8F0; border-radius: 0.5rem; height: 100%;'><span style='font-family: JetBrains Mono; font-size: 12px; font-weight: bold;'>{title}</span><br><br><span style='font-size: 14px; color: #45464d;'>{val}</span></div>",
                            unsafe_allow_html=True,
                        )
            ...
            if recs:
                st.markdown(
                    "<br>**ACTIONABLE RECOMMENDATIONS**", unsafe_allow_html=True
                )
                for rec in recs:
                    st.markdown(f"- {rec}")
            ...
            if citations:
                with st.expander("🔍 FORENSIC EVIDENCE INSPECTOR"):
                    for cid in citations:
                        ...
                        if chunk_data:
                            ...
                            st.warning(
                                f"Chunk ID `{cid}` was referenced by the model but not found in the active retrieved pool."
                            )
            else:
                st.error(
                    "⚠️ **Uncited Risk:** This risk was flagged as invalid or missing grounded citations."
                )
