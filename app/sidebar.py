...
def render_sidebar():
    ...
    with st.sidebar.expander(
        f"Upload a document into '{project_choice}'", expanded=False
    ):
        ...
        add_doc_clicked = st.button(
            "Add to Knowledge Base", disabled=uploaded_file is None
        )
            ...
            with st.spinner(f"Embedding '{uploaded_file.name}'..."):
                try:
                    ...
                    doc = build_document_from_upload(
                        uploaded_file.name, raw_text, project_choice
                    )
                    add_document_to_project(chunk_documents([doc]), project_choice)
                    logger.info(
                        "Uploaded document '%s' added to project '%s'.",
                        uploaded_file.name,
                        project_choice,
                    )
                    ...
                    st.session_state.upload_status = {
                        "ok": True,
                        "message": f"Added '{uploaded_file.name}'.",
                    }
                    ...
                    logger.exception(
                        "Upload failed for project '%s' and file '%s': %s",
                        project_choice,
