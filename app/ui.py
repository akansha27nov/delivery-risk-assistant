...
st.markdown(
    "Automated multi-angle evidence extraction, cost/impact estimation, citation gating, and deterministic HITL routing."
)
...
if run_inspection or run_audit:
    with st.spinner(f"Gathering evidence for '{project_choice.upper()}'..."):
        try:
            st.session_state.retrieved_evidence = cached_gather_evidence(
                project_choice, user_question
            )
        except Exception as e:
            logger.exception(
                "Evidence retrieval failed for project '%s': %s", project_choice, e
            )
...
if st.session_state.retrieved_evidence and not run_audit:
    ...
    for idx, chunk in enumerate(st.session_state.retrieved_evidence, 1):
        with st.expander(
            f"Chunk {idx}: `{chunk.get('chunk_id')}` | Score: {chunk.get('rerank_score', chunk.get('score', 0)):.4f}"
        ):
            st.info(chunk.get("text", "No text found"))
    st.info(
        "💡 Review evidence, then click **Step B: Run Full Risk Audit** in the sidebar."
    )
...
if run_audit:
    with st.spinner(f"Executing LangGraph pipeline for '{project_choice.upper()}'..."):
        try:
            ...
            st.session_state.audit_result = asyncio.run(
                graph.ainvoke({"project": project_choice, "question": user_question})
            )
            ...
            with open(
                os.path.join(samples_dir, f"{project_choice}_risk_report.md"),
                "w",
                encoding="utf-8",
            ) as f:
                f.write(
                    generate_sample_report_markdown(
                        project_choice, st.session_state.audit_result
                    )
                )

        except Exception as e:
            logger.exception(
                "Pipeline execution failed for project '%s': %s", project_choice, e
            )
            st.error(f"⚠️ Pipeline execution error: {e}")
            st.info(
                "💡 Tip: Verify your OpenAI and Pinecone API keys in the .env file."
            )
...
if st.session_state.audit_result:
    ...
    render_executive_summary(
        res_container.get("risks", []),
        st.session_state.retrieved_evidence,
        requires_hitl,
    )
    st.markdown("---")
    render_risk_breakdown(
        res_container.get("risks", []), st.session_state.retrieved_evidence
    )

elif not run_inspection:
    st.info(
        "👈 Use the sidebar to inspect retrieved evidence or run a full risk audit."
    )
