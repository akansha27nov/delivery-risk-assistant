...
def generate_notion_markdown(
    project: str, status: str, findings: list[dict[str, Any]], message: str
) -> str:
    """
...
async def run_pipeline(project: str, question: str) -> dict[str, Any]:
    ...
    summary = {
        ...
        "pending_review": sum(
            1 for f in processed_findings if f["status"] == "pending_hitl_approval"
        ),
        "rejected": sum(1 for f in processed_findings if not f["valid"]),
    }

    # Pre-render the full Markdown executive report
    report_md = generate_notion_markdown(
        project, overall_status, processed_findings, message
    )
...
def main():
    ...
    try:
        ...
        results = asyncio.run(
            run_pipeline(project=args.project, question=args.question)
        )
        indent = 2 if args.pretty else None
        print(json.dumps(results, indent=indent, default=str))
        logger.info(
            "Audit completed for project '%s' with status '%s'.",
            args.project,
            results["status"],
        )
