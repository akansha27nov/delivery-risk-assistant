...
async def retrieve_documents(state: GraphState) -> GraphState:
    ...
    logger.info(
        "Retrieved %d evidence chunk(s) for project '%s'.",
        len(evidence),
        state["project"],
    )
...
def validate_citations_node(state: GraphState) -> GraphState:
    ...
    logger.info(
        "Citation validation completed for %d risk(s). Invalid: %d.",
        len(validated),
        sum(1 for r in validated if not r["valid"]),
    )
...
def reject_response(state: GraphState) -> GraphState:
    ...
    for r in invalid:
        if not r.get("context_consistent", True):
            reasons.append(
                f'"{r["risk"]}" -- context mismatch: {r.get("context_mismatch_detail", "")}'
            )
        elif not r.get("citations"):
            reasons.append(f'"{r["risk"]}" -- no valid citation')
        elif not r.get("contradiction_grounded", True):
            reasons.append(
                f'"{r["risk"]}" -- claims a status contradiction but no cited chunk contains a matching status claim'
            )
        else:
            reasons.append(
                f'"{r["risk"]}" -- missing required confidence tag or impact breakdown'
            )
    ...
    logger.warning(
        "Rejecting response for project '%s': %s",
        state.get("project", "unknown"),
        message,
    )
...
def route_to_hitl(state: dict) -> dict:
    ...
    if TELEGRAM_ALERTS_ENABLED and TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
        ...
        try:
            ...
            logger.info(
                "Telegram HITL alert sent successfully for project '%s'.", project
            )
            ui_message = "High-severity risk or status contradiction detected. Escalated to Telegram for human review."
        except Exception as e:
            logger.exception(
                "Error sending Telegram notification for project '%s': %s", project, e
            )
        ...
        logger.info(
            "Telegram HITL alert suppressed for project '%s' (TELEGRAM_ALERTS_ENABLED=false).",
            project,
        )
...
def generate_report(state: GraphState) -> GraphState:
    logger.info(
        "Generated final report for project '%s' with %d risk(s).",
        state["project"],
        len(state.get("risks", [])),
    )
    return {
...
def export_workflow_diagrams(png_output_path: str = "docs/workflow_diagram.png") -> str:
    ...
    compiled_graph = (
        build_graph()
    )  # Replace with your actual compilation variable/function if different
    ...
    try:
        graph_runnable.draw_mermaid_png(
            output_file_path=png_output_path, background_color="white", padding=15
        )
        logger.info(
            "Successfully generated workflow diagram PNG at: %s", png_output_path
        )
    except Exception as e:
        logger.exception(
            "PNG generation skipped or failed (network/API required for mermaid.ink): %s",
            e,
        )
