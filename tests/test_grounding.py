...
def test_attrition_risk_if_present_is_single_sourced():
    ...
    for risk in result.get("risks", []):
        ...
        if any(
            kw in text
            for kw in ("burnout", "exploring other roles", "attrition", "retention")
        ):
            assert all(
                cid.startswith("standup_transcript.txt") for cid in risk["citations"]
            ), (
...
def test_atlas_and_nova_never_cross_contaminate():
    ...
    for risk in nova_result.get("risks", []):
        for cid in risk["citations"]:
            source = cid.split("::")[0]
            assert source not in ATLAS_SOURCES, (
                f"Nova risk cited an Atlas source: {cid}"
            )
