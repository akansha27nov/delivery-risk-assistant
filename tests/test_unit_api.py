...
def test_trigger_audit_success(mock_run_pipeline):
    ...
    response = client.post("/run-audit", json={"project": "atlas", "question": "Are there blockers?"})
    
    assert response.status_code == 200
    assert response.json() == {"status": "success", "summary": "All clear"}
    mock_run_pipeline.assert_called_once_with(project="atlas", question="Are there blockers?")
