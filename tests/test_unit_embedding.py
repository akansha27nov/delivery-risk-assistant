...
def test_ensure_index_creates_when_missing(mock_pc_cls):
    ...
    with patch.object(embedding, "pc", mock_pc), patch.object(embedding, "INDEX_NAME", "test-index"):
...
def test_build_vector_store(mock_pc_cls, mock_openai_cls):
    ...
    with patch.object(embedding, "pc", mock_pc), \
         patch.object(embedding, "openai_client", mock_openai), \
         patch.object(embedding, "INDEX_NAME", "test-index"):
        
        chunks = [{"chunk_id": "c1", "source": "a.md", "location": "loc", "text": "sample", "project": "atlas"}]
...
def test_add_document_to_project(mock_pc_cls):
    ...
    with patch.object(embedding, "pc", mock_pc), \
        ...
        chunks = [{"chunk_id": "c2", "source": "b.md", "location": "loc", "text": "new data", "project": "atlas"}]
