...
def test_load_documents_unlisted_file_raises_error(tmp_path):
    ...
    with pytest.raises(
        ValueError,
        match="is in knowledge_base/ but not listed in project_manifest.json",
    ):
...
def test_build_document_from_upload_text():
    ...
    doc = ingestion.build_document_from_upload(
        filename, raw_content=raw_content, project="atlas"
    )
...
def test_build_document_from_upload_unsupported():
    ...
    with pytest.raises(ValueError, match="Unsupported file type"):
        ingestion.build_document_from_upload(
            "archive.zip", raw_content="data", project="atlas"
        )
